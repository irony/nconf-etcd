...
NconfEtcd.prototype.get = function(key, callback) {
  // todo JSON deserialization? via try/catch
  this.etcd.get(key, function(err, resp) {
    if (err) { return callback(err); }

    return callback(null, resp.node.value);
  });
};
...
NconfEtcd.prototype.clear = function(key, callback) {
  this.etcd.del(key, callback);
};

NconfEtcd.prototype.merge = function(key, value, callback) {
  // todo - merge via _ and then set
};
